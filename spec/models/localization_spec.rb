...
describe Localization do
  ...
  it 'should have a collection of versions' do
    (FactoryGirl.create(:localization).respond_to?(:versions)).should be
    FactoryGirl.create(:localization).versions.first.should_not be_nil
  end

  describe 'a localization with two revisions' do
    before do
      @localization = FactoryGirl.create(:localization, :draft_content => 'default')
  ...
  it 'copies draft content when published' do
    ...
    version = FactoryGirl.create(:version, :localization => localization, :content => new_content)
  ...
  it 'creates the first version without counting versions' do
    ...
    localization = FactoryGirl.build(:localization)
  ...
  it 'returns a key with locale' do
    blurb = FactoryGirl.build(:blurb, :key => 'test.key')
    locale = FactoryGirl.build(:locale, :key => 'es')
    localization = FactoryGirl.build(:localization, :blurb => blurb, :locale => locale)
  ...
  context '#as_json' do
    subject { FactoryGirl.create(:localization) }
  ...
  context '#in_locale' do
    ...
    let!(:locale) { FactoryGirl.create(:locale, :project => project) }
    let!(:other_locale) { FactoryGirl.create(:locale, :project => project) }

    let!(:localizations) do
      [FactoryGirl.create(:localization, :locale => locale),
       FactoryGirl.create(:localization, :locale => locale)]
    end

    let!(:other_localization) { FactoryGirl.create(:localization, :locale => other_locale) }
