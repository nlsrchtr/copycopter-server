...
describe Version do
  ...
  it 'uses a preassigned number' do
    FactoryGirl.create(:version, :number => 3).reload.number.should == 3
  end
end
...
describe Version, 'revised without new content' do
  let(:previous_version) { FactoryGirl.create :version, :content => 'hello' }
...
describe Version, 'revised with new content' do
  let(:previous_version) { FactoryGirl.create :version }
...
describe Version, 'published' do
  subject { FactoryGirl.create(:published_version) }
